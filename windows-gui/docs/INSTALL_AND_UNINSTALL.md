...
2. **PowerShell script** (same defaults as above): from `windows-gui` run  
   `.\uninstall.ps1`  
...
```bash
bash /path/to/windows-gui/packaging/linux/uninstall_docker_runtime.sh
```
